...
/**
 * Imports files by copying them to data/files/{hash}/ directory
 */
...
/**
 * References files by storing their system path without copying
 */
async function referenceFiles(filePaths: string[], folderId?: number) {
  ...
  for (const filePath of filePaths) {
    ...
    try {
      ...
      const result = await fileOps.referenceFile(
        filename,
